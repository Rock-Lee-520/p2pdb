...
#### 共识算法
<!-- 有了非对称加密手段之后，每个人的资产安全都有保障了吗？实际上还没有，因为区块链的资产，最终是记录在区块链账本上的，就如比特币，每个人的比特币余额是由其账户上所有的未开支余额加总得到的，这本账本被同步到比特币网络的每个节点上，只有网络上每个节点都认可账本内容时，余额才真实有效，所以很长一段时间内，区块链技术也被称为分布式账本技术。

但因为这是一个开放式的网络，运行的节点都可以加入记账，也同时都有可能被篡改，所以为了对抗被篡改（一般为“双花攻击”）的攻击，必须有共识算法，即不管网络中的节点是否恶意，总有一种办法可以在网络中达成共识，达成共识之后的计算结果才能被确认。 -->
